...
const aminoAcids = {
  a: ({width, x, y, selected}) => simpleCircle({
    width: width * 0.95,
    x, y,
    strokeWidth: 7,
  ...
  r: ({width, x, y, selected}) => simpleCircle({
    width: width * 0.95,
    x, y,
    strokeWidth: 12,
    ...
    scale: width/50,
    opacity: selected ? 1 : .3}),
  c: ({width, x, y, selected}) => closedPath({
    ...
    scale: width/50,
    opacity: selected ? 1 : .3}),
  d: ({width, x, y, selected}) => closedPath({
    ...
    scale: width/50,
    opacity: selected ? 1 : .3}),
  e: ({width, x, y, selected}) => closedPath({
    ...
    scale: width/50,
    opacity: selected ? 1 : .3}),
  f: ({width, x, y, selected}) => closedPath({
...
const AminoAcid = ({type="a", width=18, x=0, y=0, selected=false}) => aminoAcids[type]({width, x, y, selected});
